  ...
  return (
    <div className="exercise">
      ...
      <div className="practice-card" style={{ marginTop: 12, fontSize: 18 }}>
        ...
        {!name ? <p>이름을 입력해 주세요</p> : <p>안녕하세요, {name}님!</p>}
      </div>

      <p className="expected">
        기대 결과: "홍길동" 입력 시 "안녕하세요, 홍길동님!"이 실시간으로
        표시됩니다.
      </p>
    </div>
  );
...
function Problem2() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        체크박스, 라디오 버튼, 셀렉트 박스를 각각 제어 컴포넌트로 만들고, 아래
        요약 영역에 현재 값을 표시하세요.
      </p>
      ...
      <p className="expected">
        기대 결과: 각 항목을 바꿀 때마다 위 JSON이 실시간으로 바뀝니다. agreed는
        true/false, gender는 'male' 또는 'female'.
      </p>
    </div>
  );
}
...
function Problem3() {
  ...
  //   HINT: email.includes('@')
  const isValidEmail = email.includes("@"); // 이 줄을 수정하세요

  // TODO 3-2: isValidPassword를 계산하세요. password.length >= 8 이면 true.
  const isValidPassword = password.length >= 8; // 이 줄을 수정하세요

  // TODO 3-3: canSubmit을 계산하세요. isValidEmail && isValidPassword 가 true일 때만 제출 가능.
  const canSubmit = isValidEmail && isValidPassword; // 이 줄을 수정하세요

  return (
    <div className="exercise">
      <h3>문제 3: 실시간 검증하며 버튼 활성화하기</h3>
      <p>
        이메일에 @가 포함되고 비밀번호가 8자 이상일 때만 "가입하기" 버튼이
        활성화되어야 합니다.
      </p>
      ...
      <p className="expected">
        기대 결과: 조건을 하나라도 못 채우면 버튼이 비활성화되고, 잘못된 값에는
        빨간 에러 문구가 바로 표시됩니다.
      </p>
    </div>
  );
...
function Problem4() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        아래 폼은 State를 전혀 쓰지 않습니다. value 대신{" "}
        <code>defaultValue</code>로 초기값을 주고, 제출 시 ref로 값을 꺼내세요.
      </p>
      ...
      <p className="expected">
        기대 결과: 타이핑 중에는 아무것도 안 보이고, "제출" 클릭 시에만 현재
        input 값이 아래에 표시됩니다.
      </p>
    </div>
  );
