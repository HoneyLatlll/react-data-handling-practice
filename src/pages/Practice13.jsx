      ...
      {file && (
        <div className="practice-card" style={{ marginTop: 12 }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--text)' }}>
            크기: {(file.size / 1024).toFixed(2)} KB · 타입: {file.type || '(알 수 없음)'}
          </p>
        </div>
      )}
...
function Problem2() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        이미지가 아닌 파일 또는 5MB를 넘는 파일을 선택하면 빨간 에러 문구가 뜨고,
        통과한 경우엔 미리보기가 보여야 합니다.
      </p>
      ...
      {error && (
        <p style={{ color: 'crimson', marginTop: 8, fontSize: 13 }}>⚠️ {error}</p>
      )}
      ...
      <p className="expected">
        기대 결과: 5MB 이하 이미지만 미리보기와 함께 표시되고, 조건에 맞지 않으면 에러 메시지가 표시됩니다.
      </p>
    </div>
  )
}
...
function Problem3() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        <code>multiple</code> 속성을 붙여 여러 파일을 한 번에 선택하고, 각 항목 옆 "❌" 버튼으로 삭제할 수 있게 만드세요.
      </p>
      ...
      <ul className="practice-list" style={{ marginTop: 12 }}>
        {files.map((file, index) => (
          <li key={`${file.name}-${index}`}>
            <span>
              📄 {file.name}
              <span style={{ color: 'var(--text)', fontSize: 12, marginLeft: 8 }}>
      ...
      <p className="expected">
        기대 결과: 여러 파일을 한 번에 선택하면 목록이 보이고, ❌ 버튼으로 개별 삭제가 가능합니다.
      </p>
    </div>
  )
}
...
function Problem4() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        파일 + 설명을 한 번에 FormData로 조립해서, 어떤 엔트리가 포함되는지 확인하세요.
      </p>
      ...
      {formDataEntries.length > 0 && (
        <div className="practice-card" style={{ marginTop: 12 }}>
          ...
          <ul style={{ fontFamily: 'var(--mono)', fontSize: 13, paddingLeft: 20 }}>
      ...
      <p className="expected">
        기대 결과: "FormData 만들기" 클릭 시 file(파일 정보)과 description(텍스트) 두 엔트리가 나열됩니다.
      </p>
    </div>
  )
}
