  ...
  return (
    <div className="exercise">
      ...
      <p className="expected">
        기대 결과: "포커스" 클릭 시 input에 커서가 자동으로 이동하고, 바로
        타이핑할 수 있습니다.
      </p>
    </div>
  );
...
function Problem2() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        이벤트 핸들러가 분리된 구조라 setInterval ID를 컴포넌트 어디서든 꺼낼 수
        있어야 합니다. useRef에 저장하면 리렌더링 없이 유지됩니다.
      </p>
      ...
      <p className="expected">
        기대 결과: "시작"을 여러 번 눌러도 속도가 빨라지지 않고 1초에 1씩
        증가합니다. "정지" 후 다시 "시작" 가능, "리셋"은 0으로 초기화합니다.
      </p>
    </div>
  );
}
...
function Problem3() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        버튼 클릭 시 아래 목록의 마지막 항목까지 부드럽게 스크롤되어야 합니다.
      </p>
...
function Problem4() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        왼쪽 버튼은 state를 증가시켜 화면이 즉시 갱신됩니다. 오른쪽 버튼은 ref만
        증가시키므로 화면이 바뀌지 않습니다. "강제 리렌더링" 버튼을 누르면
        그동안 쌓인 ref 값이 화면에 반영됩니다.
      </p>
      ...
      <div className="practice-card" style={{ textAlign: "center" }}>
        ...
        <p style={{ fontSize: 18 }}>
          ref count (렌더링 시점):
          {/* TODO 4-3: 아래에 refCount.current를 표시하세요. */}
          {refCount.current}
        </p>
      </div>
      ...
      <p className="expected">
        기대 결과: "ref +1"만 여러 번 눌러도 화면의 ref count는 그대로이고,
        콘솔에만 값이 증가합니다. "강제 리렌더링"을 누르면 그동안 쌓인 ref 값이
        화면에 표시됩니다.
      </p>
    </div>
  );
