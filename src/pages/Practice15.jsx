...
function Practice15() {
  return (
    <div className="page">
      ...
      >
        💡 JSONPlaceholder는 실제로 서버 데이터를 바꾸지 않고 "성공 응답"만
        돌려줍니다. 이 실습에서는 응답 객체를 받아 로컬 state를 업데이트하는
        패턴에 집중하세요.
      </div>
...
function Problem1() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        폼을 제출하면 서버가 돌려준 id(예: 101)를 포함한 새 게시글이 목록 맨
        위에 즉시 표시되어야 합니다.
      </p>
      ...
      <p className="expected">
        기대 결과: 작성 버튼 클릭 시 폼이 비워지고, id가 101인 새 게시글이 목록
        맨 앞에 추가됩니다.
      </p>
    </div>
  );
...
function Problem2() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        각 게시글의 "수정" 버튼을 눌러 제목·본문을 편집하고 저장하세요. 저장 시
        해당 항목만 업데이트됩니다.
      </p>
      ...
      <p className="expected">
        기대 결과: "수정" 클릭 시 해당 항목만 인라인 편집 폼으로 바뀌고, "저장"
        시 목록의 해당 항목만 새 값으로 교체됩니다.
      </p>
    </div>
  );
}
...
function Problem3() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        "삭제" 버튼 클릭 시 확인 창을 띄우고, DELETE 요청 성공 시 로컬
        목록에서만 제거하세요.
      </p>
      ...
      <p className="expected">
        기대 결과: 확인 창에서 "확인"을 누르면 해당 항목이 목록에서 사라집니다.
        네트워크 탭에서 DELETE 요청이 한 번 보내집니다.
      </p>
    </div>
  );
}
...
function Problem4() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        DELETE 요청 후 <code>loadPosts()</code>를 다시 호출해 목록을 통째로 새로
        받아오세요.
      </p>
      ...
      <p className="expected">
        기대 결과: 삭제 시 DELETE + GET 두 요청이 나갑니다. 호출 횟수 카운터가
        1씩 올라갑니다. (JSONPlaceholder 특성상 GET 응답은 원본 그대로
        돌아옵니다.)
      </p>
    </div>
  );
