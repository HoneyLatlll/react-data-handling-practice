...
function Problem1() {
  ...
  return (
    <div className="exercise">
      ...
      <p>"더 보기" 버튼을 누를 때마다 10개씩 게시글이 목록에 추가되어야 합니다.</p>
      ...
      <p className="expected">
        기대 결과: 최초 진입 시 10개 → "더 보기" 1회 → 20개 → 2회 → 30개 ... 누적 표시됩니다.
      </p>
    </div>
  )
}
...
function Problem2() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        전체 100개를 다 로드하고 나면 "더 보기" 버튼을 숨기고 "모든 게시물을 불러왔습니다" 메시지를 표시하세요.
      </p>
      ...
      <div className="toolbar" style={{ marginTop: 12 }}>
        ...
        {/* HINT: 삼항 연산자 사용 */}
        <button onClick={() => setOffset((prev) => prev + LIMIT)}>더 보기</button>
      </div>

      <p className="expected">
        기대 결과: 10번 정도 "더 보기"를 누르면 100개를 모두 로드하고, 버튼이 완료 메시지로 바뀝니다.
      </p>
    </div>
  )
}
...
const TOTAL_PAGES = TOTAL_POSTS / SIZE
...
function Problem3() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        "이전" / "다음" 버튼으로 페이지를 이동하세요. 첫 페이지에서는 "이전", 마지막 페이지에서는 "다음" 버튼이 disabled 되어야 합니다.
      </p>
      ...
      <p className="expected">
        기대 결과: 이전/다음으로 페이지 이동 시 목록이 완전히 교체되며, 가장자리에선 해당 버튼이 비활성화됩니다.
      </p>
    </div>
  )
}
...
function Problem4() {
  ...
  return (
    <div className="exercise">
      ...
      <p>
        1 ~ {TOTAL_PAGES}번까지의 페이지 버튼을 <code>Array.from</code>으로 생성하세요.
        현재 페이지 버튼은 <code>.active</code> 클래스로 강조합니다.
      </p>
      ...
      <p className="expected">
        기대 결과: 1 ~ {TOTAL_PAGES}번 페이지 버튼이 보이고, 클릭 시 해당 페이지 데이터로 교체되며,
        현재 페이지 버튼은 보라색(active)로 강조됩니다.
      </p>
    </div>
  )
}
